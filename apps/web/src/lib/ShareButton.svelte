...
<div class="share-control">
  <button class:copied={message === 'Copied to clipboard'} class="share-button" type="button" on:click={share} aria-label={media ? 'Share media' : 'Share discussion'}>{message === 'Copied to clipboard' ? '✓ Copied' : '↗ Share'}</button>
  {#if message}<span role="status" aria-live="polite">{message}</span>{/if}
  {#if shareUrl && message === 'Copy the link below'}<input readonly value={shareUrl} aria-label={media ? 'Media link' : 'Discussion link'} on:focus={(event) => event.currentTarget.select()} />{/if}
</div>
<style>
