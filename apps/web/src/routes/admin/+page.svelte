<script>
  ...
  let drawExecutable = 'draw-things-cli', drawModelsDir = '', drawModel = '', drawWidth = 1024, drawHeight = 1024, drawSteps = 4, drawCfg = '', drawSeed = '', drawLoras = '[]', drawLoraRows = [], drawPromptPermutationRows = [], drawOutputPath = '', drawPostsPerRun = 1, drawTitlePrefix = 'Draw Things generation';
  let crossMode = 'api', crossAccounts = '', crossTopics = '', crossQueries = '', crossHours = 24, crossLimit = 8, crossPerSource = 50, crossStartTime = '', crossEndTime = '', crossIncludeReplies = false, crossIncludeRetweets = false;
  ...
  const backupStateLabel = value => value === 'critical' ? 'Budget exceeded' : value === 'warning' ? 'Near budget' : value === 'healthy' ? 'Within budget' : 'No backup data';
  const backupPercent = backups => {
    const total = Number(backups?.total_archive_bytes), budget = Number(backups?.budget_bytes);
    return Number.isFinite(total) && Number.isFinite(budget) && budget > 0 ? Math.min(100, Math.max(0, total / budget * 100)) : 0;
  };
  const size = value => {
  ...
  const runnerKinds = { draw_things: 'Draw Things', command: 'Command runner', cross_post: 'Cross-post adapter', content_package: 'Content package extension' };
  const runnerKindDescriptions = { draw_things: 'Generate images on this host', command: 'Run an allow-listed command', cross_post: 'Collect public X posts into a bounded batch', content_package: 'Run an optional pack with JSONL progress and checkpoints' };
  const runnerTemplates = [
    ...
    { key: 'x-recommended', kind: 'cross_post', name: 'X Recommended · one unique link', description: 'Reads one unseen public status from the signed-in For You timeline through a dedicated read-only Ego Lite session.', command: ['node', 'scripts/x-recommended-session-runner.mjs'], prompt: 'Collect one unseen public post from the X Recommended timeline.', interval: 60, environmentKeys: 'EGO_BROWSER_SPACE_ID, EGO_BROWSER_CLI' },
    { key: 'x-playwright-recommended', kind: 'cross_post', name: 'X Recommended · Ubuntu Playwright', description: 'Reads one unseen public status from a dedicated signed-in Chromium profile on an Ubuntu worker, without taking any X account actions.', command: ['node', 'scripts/x-playwright-recommended-runner.mjs'], prompt: 'Collect one unseen public post from the X Recommended timeline.', interval: 600, environmentKeys: 'X_PLAYWRIGHT_USER_DATA_DIR, X_PLAYWRIGHT_HEADLESS, X_PLAYWRIGHT_EXECUTABLE_PATH' },
    ...
    { key: 'draw-things-variants', kind: 'draw_things', name: 'Draw Things variants', description: 'Generates two deterministic prompt variants per run so you can compare output, timing, and memory pressure before enabling it.', command: { executable: 'draw-things-cli', model: 'flux_1_schnell_q5p.ckpt', width: 1024, height: 1024, steps: 4, cfg: null, seed: 42, loras: [], prompt_permutations: [{ key: 'lighting', values: ['soft morning light', 'golden hour light'] }], posts_per_run: 2, output_path: '~/Library/Application Support/Swartzit/generated/variant-{runner_id}-{timestamp}-{index}.png', title_prefix: 'Draw Things · variant' }, prompt: 'A clean editorial illustration for {community}; {lighting}; variant {index} of {total}; runner {runner}; {date}.', interval: 604800 },
    ...
    { key: 'draw-things-validated', kind: 'draw_things', name: 'Draw Things validated workflow', description: 'A complete test recipe: LoRA, dynamic built-in tokens, a 2×2 prompt permutation matrix, deterministic seeds, and post metadata.', command: { executable: 'draw-things-cli', model: 'flux_1_dev_q8p.ckpt', width: 1024, height: 1024, steps: 28, cfg: 3.5, seed: 123, loras: [{ file: 'flux_alexandra_daddario_lora_f16.ckpt', version: 'flux1', weight: 0.8 }], prompt_permutations: [{ key: 'lighting', values: ['soft daylight', 'golden hour'] }, { key: 'composition', values: ['wide editorial frame', 'close portrait frame'] }], posts_per_run: 4, output_path: '~/DrawThings/validated/draw-{runner_id}-{timestamp}-{index}.png', title_prefix: 'Draw Things · validated' }, prompt: 'A polished editorial portrait for {community}; {lighting}; {composition}; {weekday}, {date}; variant {index} of {total}.', interval: 604800 },
    { key: 'starwars-pack', kind: 'content_package', name: 'Gravedancer to General pack', description: 'Optional local story extension. Generates one checkpointed article unit per day with Gemma defaults and publishes each day as its own article with its matching image.', command: { pack: 'starwars.gravedancer', argv: ['python3', 'scripts/swartzit_pack_runner.py', '--pack', 'starwars.gravedancer'], working_dir: '../Starwars_Anatomy_of_a_Catastrophe_Gravedancer_to_General', options: { seed: 42, days: 7, fast: true, model: 'mlx-community/gemma-4-e4b-it-OptiQ-4bit', generate_images: false, publish_mode: 'article_units', draw_things: { backend: 'cli', model: 'flux_1_schnell_q5p.ckpt', width: 1024, height: 576, steps: 4, cfg: 1.4, loras: [] } } }, prompt: 'Generate the next Star Wars episode from the configured pack seed.', interval: 604800 }
  ];
  ...
  const runStateTone = value => ['success', 'completed'].includes(value) ? 'positive' : ['failed', 'error'].includes(value) ? 'danger' : ['running', 'claimed', 'retrying', 'paused'].includes(value) ? 'warning' : 'neutral';
  ...
  function runnerRecipeLabel(runner) {
    ...
    if (runner.kind === 'cross_post') return Array.isArray(runner.command) && runner.command.includes('scripts/x-playwright-recommended-runner.mjs') ? 'X Recommended · Playwright' : Array.isArray(runner.command) && runner.command.includes('scripts/x-recommended-session-runner.mjs') ? 'X Recommended · Ego Lite' : 'X public API';
    if (runner.kind === 'content_package') return config.pack || 'Pack not configured';
  ...
  function runnerRecipeDetail(runner) {
    ...
    if (runner.kind === 'draw_things') {
      ...
      const permutationCount = drawPermutationCount(config.prompt_permutations);
      const variants = permutationCount ? `${permutationCount} prompt permutations` : Number(config.posts_per_run || 1) > 1 ? `${config.posts_per_run} variants` : '1 variant';
      return [dimensions, steps, guidance, lora, variants].join(' · ');
    }
    if (runner.kind === 'cross_post') return Array.isArray(runner.command) && runner.command.includes('scripts/x-playwright-recommended-runner.mjs') ? 'Ubuntu Chromium · persistent profile · read-only' : Array.isArray(runner.command) && runner.command.includes('scripts/x-recommended-session-runner.mjs') ? 'Mac Ego Lite · persistent space · read-only' : 'Bounded X source window · worker-side token';
    if (runner.kind === 'content_package') {
      const units = config.options?.days ? `${config.options.days} planned days` : 'checkpointed units';
      return `${units} · ${config.publish_mode || config.options?.publish_mode || 'feed item'} · external adapter`;
    }
  ...
  function runnerVariantCount(runner) { const config = runner.command && typeof runner.command === 'object' && !Array.isArray(runner.command) ? runner.command : {}; return drawPermutationCount(config.prompt_permutations) || Number(config.posts_per_run || 1); }
  function testPreviews(runner) { const run = latestRun(runner); return run.dry_run && run.status === 'success' && Array.isArray(run.detail?.previews) ? run.detail.previews : []; }
  function testPreviewSettings(preview) { const config = preview?.generation_config || {}; const loras = Array.isArray(config.loras) ? config.loras.length : 0; const permutation = config.permutation && config.prompt_variables ? ` · permutation ${config.permutation.index}/${config.permutation.total}` : ''; return `${config.model || 'model not recorded'} · ${config.width || '—'}×${config.height || '—'} · ${config.steps || '—'} steps · ${loras} LoRA${loras === 1 ? '' : 's'}${permutation}`; }
  ...
  function latestRunError(runner) { return String(latestRun(runner).error || runner.last_error || '').trim(); }
  ...
  async function revealRunnerEditor() { await tick(); runnerEditorElement?.scrollIntoView({ behavior: 'smooth', block: 'start' }); runnerNameInput?.focus(); }
  function openNewRunner() { resetRunnerForm(); runnerEditorMode = 'new'; runnerFormError = ''; runnerEditorOpen = true; void revealRunnerEditor(); }
  ...
  function setDrawLoras(value) { drawLoraRows = normalizedLoras(value); drawLoras = JSON.stringify(drawLoraRows); }
  function addDrawLora() { drawLoraRows = [...drawLoraRows, { file: '', version: 'flux1', weight: 0.8 }]; drawLoras = JSON.stringify(drawLoraRows); }
  function updateDrawLora(index, key, value) { drawLoraRows = drawLoraRows.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: key === 'weight' ? (value === '' ? '' : Number(value)) : value } : row); drawLoras = JSON.stringify(drawLoraRows); }
  function removeDrawLora(index) { drawLoraRows = drawLoraRows.filter((_, rowIndex) => rowIndex !== index); drawLoras = JSON.stringify(drawLoraRows); }
  function normalizedPromptPermutations(value) { return (Array.isArray(value) ? value : []).map(item => ({ key: String(item?.key ?? ''), values: Array.isArray(item?.values) ? item.values.map(option => String(option ?? '')) : [] })); }
  function setDrawPromptPermutations(value) { drawPromptPermutationRows = normalizedPromptPermutations(value); }
  function addDrawPromptPermutation() { drawPromptPermutationRows = [...drawPromptPermutationRows, { key: '', values: [''] }]; }
  function updateDrawPromptPermutationKey(index, value) { drawPromptPermutationRows = drawPromptPermutationRows.map((row, rowIndex) => rowIndex === index ? { ...row, key: value } : row); }
  function updateDrawPromptPermutationValues(index, value) { drawPromptPermutationRows = drawPromptPermutationRows.map((row, rowIndex) => rowIndex === index ? { ...row, values: value.split(/\r?\n/) } : row); }
  function removeDrawPromptPermutation(index) { drawPromptPermutationRows = drawPromptPermutationRows.filter((_, rowIndex) => rowIndex !== index); }
  function drawPermutationCount(rows = drawPromptPermutationRows) { const valid = rows.filter(row => row.key.trim() && row.values.length && row.values.every(value => value.trim())); return valid.length ? valid.reduce((total, row) => total * row.values.length, 1) : 0; }
  $: drawPermutationTotal = drawPermutationCount();
  function useRunnerTemplate(template) { resetRunnerForm(); runnerFormError = ''; runnerEditorMode = 'new'; runnerEditorOpen = true; runnerKind = template.kind || 'command'; runnerName = template.name; runnerPrompt = template.prompt; runnerInterval = template.interval; runnerEnvironmentKeys = template.environmentKeys || ''; if (template.kind === 'draw_things') { drawExecutable = template.command.executable; drawModelsDir = template.command.models_dir || ''; drawModel = template.command.model; drawWidth = template.command.width; drawHeight = template.command.height; drawSteps = template.command.steps; drawCfg = template.command.cfg; drawSeed = template.command.seed == null ? '' : template.command.seed; setDrawLoras(template.command.loras || []); setDrawPromptPermutations(template.command.prompt_permutations || []); drawOutputPath = template.command.output_path || ''; drawPostsPerRun = template.command.posts_per_run; drawTitlePrefix = template.command.title_prefix || 'Draw Things generation'; } else if (template.kind === 'cross_post') { loadCrossPostCommand(template.command); } else if (template.kind === 'content_package') { packagePack = template.command.pack; packageWorkingDir = template.command.working_dir || ''; packageOptions = JSON.stringify(template.command.options || {}, null, 2); runnerCommand = JSON.stringify(template.command.argv); } else { runnerCommand = JSON.stringify(template.command); } void revealRunnerEditor(); }
  function duplicateRunner(runner) {
    loadRunnerIntoForm(runner);
    editingRunner = null;
    runnerEditorMode = 'duplicate';
    runnerName = nextRunnerCopyName(runner.name, runners.map(item => item.name));
    runnerFormError = '';
    runnerEditorOpen = true;
    notice = `Copy of “${runner.name}” is ready as a disabled draft. Review it, then save.`;
    void revealRunnerEditor();
  }
  ...
  function contentPackageCommand() { let options = {}; if (packageOptions.trim()) { options = JSON.parse(packageOptions); if (!options || typeof options !== 'object' || Array.isArray(options)) throw new Error('Package options must be a JSON object.'); } return { pack: packagePack.trim(), argv: parseCommand(runnerCommand), ...(packageWorkingDir.trim() ? { working_dir: packageWorkingDir.trim() } : {}), options }; }
  function runnerPayload(source) { return { name: source.name, kind: source.kind, command: source.kind === 'draw_things' ? drawCommand() : source.kind === 'cross_post' ? crossPostCommand() : source.kind === 'content_package' ? contentPackageCommand() : parseCommand(source.command), prompt: source.prompt, author: source.author, community: source.community, interval_seconds: Number(source.interval), days_of_week: runnerDays, priority: Number(source.priority), timeout_seconds: Number(source.timeout), max_attempts: Number(source.attempts), retry_backoff_seconds: Number(source.backoff), failure_threshold: Number(source.threshold), retention_days: Number(source.retention), environment_keys: source.environmentKeys.split(',').map(value => value.trim()).filter(Boolean), capture_output: source.captureOutput, max_log_bytes: Number(source.maxLogBytes) }; }
  function resetRunnerForm() { runnerName = ''; runnerKind = 'command'; runnerCommand = ''; runnerPrompt = ''; runnerAuthor = ''; runnerCommunity = ''; runnerInterval = 1800; runnerDays = [1, 2, 3, 4, 5, 6, 7]; runnerPriority = 100; runnerTimeout = 900; runnerAttempts = 3; runnerBackoff = 60; runnerThreshold = 3; runnerRetention = 30; runnerEnvironmentKeys = ''; runnerCaptureOutput = true; runnerMaxLogBytes = 20000; packagePack = 'starwars.gravedancer'; packageWorkingDir = ''; packageOptions = '{}'; crossMode = 'api'; crossAccounts = ''; crossTopics = ''; crossQueries = ''; crossHours = 24; crossLimit = 8; crossPerSource = 50; crossStartTime = ''; crossEndTime = ''; crossIncludeReplies = false; crossIncludeRetweets = false; drawExecutable = 'draw-things-cli'; drawModelsDir = ''; drawModel = ''; drawWidth = 1024; drawHeight = 1024; drawSteps = 4; drawCfg = ''; drawSeed = ''; drawLoraRows = []; drawPromptPermutationRows = []; drawLoras = '[]'; drawOutputPath = ''; drawPostsPerRun = 1; drawTitlePrefix = 'Draw Things generation'; editingRunner = null; runnerEditorMode = 'new'; }
  async function saveRunner(event) { event.preventDefault(); busy = true; notice = ''; runnerFormError = ''; const testAfterSave = event.submitter?.value === 'test'; const wasEditing = Boolean(editingRunner); const wasDuplicate = runnerEditorMode === 'duplicate'; const source = { name: runnerName, kind: runnerKind, command: runnerCommand, prompt: runnerPrompt, author: runnerAuthor, community: runnerCommunity, interval: runnerInterval, priority: runnerPriority, timeout: runnerTimeout, attempts: runnerAttempts, backoff: runnerBackoff, threshold: runnerThreshold, retention: runnerRetention, environmentKeys: runnerEnvironmentKeys, captureOutput: runnerCaptureOutput, maxLogBytes: runnerMaxLogBytes }; let savedRunner; try { savedRunner = await api(editingRunner ? 'content-runners/' + editingRunner.id : 'content-runners', 'POST', runnerPayload(source)); } catch (e) { runnerFormError = e.message || 'The runner could not be saved.'; busy = false; return; } if (testAfterSave) { try { await api('content-runners/' + (savedRunner?.id ?? editingRunner?.id) + '/test', 'POST'); } catch (e) { resetRunnerForm(); runnerEditorOpen = false; notice = `Runner saved, but the no-publish test could not be queued: ${e.message || 'unknown error'}`; await refresh(); busy = false; return; } notice = wasEditing ? 'Saved and queued a no-publish test. Watch the runner card for Test queued → Test running → Success.' : wasDuplicate ? 'Copy saved as a draft and queued a no-publish test. Watch the runner card for Test queued → Test running → Success.' : 'Saved as a draft and queued a no-publish test. Watch the runner card for Test queued → Test running → Success.'; } else { notice = wasEditing ? 'Runner configuration updated.' : wasDuplicate ? 'Copy saved as a disabled draft. Test it before enabling the schedule.' : 'Runner saved as a draft. Test it before enabling the schedule.'; } resetRunnerForm(); runnerEditorOpen = false; await refresh(); busy = false; }
  function loadRunnerIntoForm(runner) { runnerName = runner.name; runnerKind = runner.kind; runnerCommand = runner.kind === 'draw_things' || runner.kind === 'cross_post' ? '' : runner.kind === 'content_package' ? JSON.stringify(runner.command?.argv || []) : JSON.stringify(runner.command); runnerPrompt = runner.prompt; runnerAuthor = runner.author; runnerCommunity = runner.community; runnerInterval = runner.interval_seconds; runnerDays = Array.isArray(runner.days_of_week) && runner.days_of_week.length ? runner.days_of_week : [1, 2, 3, 4, 5, 6, 7]; runnerPriority = runner.priority; runnerTimeout = runner.timeout_seconds; runnerAttempts = runner.max_attempts; runnerBackoff = runner.retry_backoff_seconds; runnerThreshold = runner.failure_threshold; runnerRetention = runner.retention_days; runnerEnvironmentKeys = (runner.environment_keys || []).join(', '); runnerCaptureOutput = runner.capture_output !== false; runnerMaxLogBytes = runner.max_log_bytes || 20000; if (runner.kind === 'draw_things') { const config = runner.command || {}; drawExecutable = config.executable || 'draw-things-cli'; drawModelsDir = config.models_dir || ''; drawModel = config.model || ''; drawWidth = config.width || 1024; drawHeight = config.height || 1024; drawSteps = config.steps || 4; drawCfg = config.cfg ?? ''; drawSeed = config.seed == null ? '' : config.seed; setDrawLoras(config.loras || []); setDrawPromptPermutations(config.prompt_permutations || []); drawOutputPath = config.output_path || ''; drawPostsPerRun = config.posts_per_run || 1; drawTitlePrefix = config.title_prefix || 'Draw Things generation'; } else if (runner.kind === 'cross_post') loadCrossPostCommand(runner.command); else if (runner.kind === 'content_package') { const config = runner.command || {}; packagePack = config.pack || 'starwars.gravedancer'; packageWorkingDir = config.working_dir || ''; packageOptions = JSON.stringify(config.options || {}, null, 2); } }
  function editRunner(runner) { loadRunnerIntoForm(runner); editingRunner = runner; runnerEditorMode = 'edit'; runnerFormError = ''; runnerEditorOpen = true; void revealRunnerEditor(); }
  function retryableRunner(runner) { return isFailedRunnerRun(latestRun(runner).status); }
  async function retryRunner(runner) { const latest = latestRun(runner); if (!latest.id) return; await action('content-runner-runs/' + latest.id + '/replay', 'Retry the latest failed run? This re-enables the runner and queues it now.'); }
  ...
  async function controlRunner(run, actionName) { if (!run?.id) return; const labels = { pause: 'Pause this package after its current checkpoint?', cancel: 'Cancel this package run and keep its latest checkpoint?', resume: 'Resume this package from its latest checkpoint?' }; if (!window.confirm(labels[actionName])) return; busy = true; notice = ''; try { await api('content-runner-runs/' + run.id + '/control', 'POST', { action: actionName }); notice = actionName === 'pause' ? 'Pause requested. The adapter will stop at its latest completed checkpoint.' : actionName === 'resume' ? 'Resume queued from the saved checkpoint.' : 'Cancellation requested. The latest checkpoint is retained.'; await refresh(); } catch (e) { notice = e.message; } finally { busy = false; } }
  ...
  {:else}
    ...
    {:else if tab === 'Content Runners'}
      <div class="runner-page">
        ...
        {#if runnerEditorOpen}
          <section id="runner-editor" bind:this={runnerEditorElement} class="runner-editor-shell panel">
            <div class="runner-editor-heading"><div><p class="runner-eyebrow">RUNNER CONFIGURATION <span class="runner-editor-mode">{runnerEditorMode === 'duplicate' ? 'DUPLICATE' : editingRunner ? 'EDIT' : 'NEW'}</span></p><h3>{runnerEditorMode === 'duplicate' ? 'Duplicate and edit' : editingRunner ? 'Edit runner' : 'Create runner'}</h3><p class="muted">This is the same editable configuration used for new runners, existing runners, and copies. Change any value, then save the definition or queue a no-publish test.</p></div><button type="button" class="runner-close" aria-label="Close runner editor" onclick={closeRunnerEditor}>×</button></div>
            {#if runnerEditorMode === 'duplicate'}<div class="runner-copy-note"><strong>Copy mode</strong><span>Everything below is editable. Saving creates a new disabled draft; the original runner will not be changed.</span></div>{/if}
            ...
            <form class="runner-form" onsubmit={saveRunner}>
              <div class="runner-form-main">
                <fieldset class="runner-form-section"><legend>Identity</legend><div class="runner-form-grid"><label>Runner name<input bind:this={runnerNameInput} bind:value={runnerName} placeholder="Morning field notes" required maxlength="80" /></label><label>Runner type<select aria-label="Runner type" bind:value={runnerKind}><option value="draw_things">Draw Things image generator</option><option value="content_package">Content package extension</option><option value="command">Command / generator</option><option value="cross_post">Cross-post adapter</option></select><small>{runnerKindDescriptions[runnerKind]}</small></label><label>Author handle<input bind:value={runnerAuthor} placeholder="editor" required /></label><label>Community slug<input bind:value={runnerCommunity} placeholder="general" required /></label></div></fieldset>
                <fieldset class="runner-form-section"><legend>Recipe</legend><label>{runnerKind === 'draw_things' ? 'Generation prompt' : runnerKind === 'content_package' ? 'Pack request / prompt' : 'Runner prompt'}<textarea class="runner-prompt" placeholder="What should this runner create?" bind:value={runnerPrompt} required maxlength="20000"></textarea></label><p class="field-help">Dynamic tokens are expanded at run time: <code>&#123;date&#125;</code> <code>&#123;time&#125;</code> <code>&#123;weekday&#125;</code> <code>&#123;runner&#125;</code> <code>&#123;community&#125;</code> <code>&#123;author&#125;</code> <code>&#123;seed&#125;</code> <code>&#123;index&#125;</code>/<code>&#123;total&#125;</code>. Unknown tokens are left unchanged.</p>
                  {#if runnerKind === 'draw_things'}
                    <div class="runner-subsection runner-prompt-permutation-editor"><div class="runner-subsection-heading"><div><strong>Prompt permutations</strong><span>{drawPermutationTotal ? `${drawPermutationTotal} combinations` : 'Optional matrix'}</span></div><button type="button" class="runner-secondary runner-add-button" onclick={addDrawPromptPermutation}>＋ Add variable</button></div>{#if drawPromptPermutationRows.length === 0}<p class="field-help">Optional. Add a variable such as <code>lighting</code>, put <code>&#123;lighting&#125;</code> in the prompt, and define one value per line.</p>{:else}<div class="runner-permutation-list">{#each drawPromptPermutationRows as permutation, index}<div class="runner-permutation-row"><label>Token<input value={permutation.key} placeholder="lighting" oninput={(event) => updateDrawPromptPermutationKey(index, event.currentTarget.value)} /></label><label>Values<textarea rows="2" value={permutation.values.join('\n')} placeholder="soft daylight&#10;golden hour" oninput={(event) => updateDrawPromptPermutationValues(index, event.currentTarget.value)}></textarea><small>One value per line. Use &#123;{permutation.key || 'token'}&#125; in the prompt.</small></label><button type="button" class="runner-text-action runner-remove-button" onclick={() => removeDrawPromptPermutation(index)}>Remove</button></div>{/each}</div><p class="field-help">Every combination is generated in stable order, up to 8 total. Built-in tokens remain available alongside these variables.</p>{/if}</div>
                    <div class="runner-subsection"><div class="runner-subsection-heading"><strong>Draw Things settings</strong><span>Saved with every generated post</span></div><div class="runner-form-grid runner-draw-grid"><label>Executable<input bind:value={drawExecutable} placeholder="draw-things-cli" /></label><label>Model<input list="draw-model-options" bind:value={drawModel} placeholder="flux_1_schnell_q5p.ckpt" required /><datalist id="draw-model-options"><option value="flux_1_schnell_q5p.ckpt"></option><option value="flux_1_dev_q8p.ckpt"></option></datalist></label><label>Models directory<input bind:value={drawModelsDir} placeholder="Default Draw Things Models directory" /></label><label>Output path / template<input bind:value={drawOutputPath} placeholder="~/DrawThings/lighthouse-{index}.png" /></label><label>Width<input type="number" min="64" max="4096" bind:value={drawWidth} /></label><label>Height<input type="number" min="64" max="4096" bind:value={drawHeight} /></label><label>Steps<input type="number" min="1" max="200" bind:value={drawSteps} /></label><label>CFG (optional)<input type="number" min="0" max="50" step="0.1" bind:value={drawCfg} placeholder="Model recommended" /></label><label>Starting seed<input type="number" min="0" bind:value={drawSeed} placeholder="automatic" /></label><label>Posts per run<input type="number" min="1" max="8" bind:value={drawPostsPerRun} /></label><label class="span-two">Title prefix<input bind:value={drawTitlePrefix} placeholder="Draw Things generation" /></label></div><div class="runner-lora-editor"><div class="runner-subsection-heading"><div><strong>LoRAs</strong><span>Optional style adapters</span></div><button type="button" class="runner-secondary runner-add-button" onclick={addDrawLora}>＋ Add LoRA</button></div>{#if drawLoraRows.length === 0}<p class="field-help">No LoRAs configured. Add one when the recipe needs a style adapter.</p>{:else}<div class="runner-lora-list">{#each drawLoraRows as lora, index}<div class="runner-lora-row"><label>File<input value={lora.file} placeholder="style.ckpt" oninput={(event) => updateDrawLora(index, 'file', event.currentTarget.value)} /></label><label>Version<select value={lora.version} onchange={(event) => updateDrawLora(index, 'version', event.currentTarget.value)}><option value="flux1">flux1</option><option value="sdxl">sdxl</option><option value="sd15">sd15</option><option value="">Custom / blank</option></select></label><label>Weight<input type="number" min="-2" max="2" step="0.05" value={lora.weight} placeholder="0.8" oninput={(event) => updateDrawLora(index, 'weight', event.currentTarget.value)} /></label><button type="button" class="runner-text-action runner-remove-button" onclick={() => removeDrawLora(index)}>Remove</button></div>{/each}</div>{/if}</div><p class="field-help">The worker invokes the local CLI without a shell. Leave CFG blank to keep the model’s recommended guidance. For multiple posts, <code>&#123;index&#125;</code> in the output path and prompt becomes the one-based variant number; <code>&#123;timestamp&#125;</code> makes each run unique. Test after saving to generate files without publishing.</p></div>
                  {:else if runnerKind === 'content_package'}
                    <div class="runner-subsection"><div class="runner-subsection-heading"><strong>Optional pack adapter</strong><span>JSONL checkpoints</span></div><div class="runner-form-grid runner-cross-grid"><label>Pack ID<input bind:value={packagePack} placeholder="starwars.gravedancer" required /><small>Versioned pack name, not a server plugin.</small></label><label>Working directory<input bind:value={packageWorkingDir} placeholder="../Starwars_Anatomy_of_a_Catastrophe_Gravedancer_to_General" /><small>Host-local directory containing the adapter.</small></label><label class="span-two">Adapter command argv JSON<textarea class="runner-command" rows="2" placeholder='["python3", "scripts/swartzit_pack_runner.py", "--pack", "starwars.gravedancer"]' bind:value={runnerCommand} required></textarea></label><label class="span-two">Pack options JSON<textarea class="runner-command" rows="8" placeholder='&#123;&quot;seed&quot;:42,&quot;days&quot;:7,&quot;fast&quot;:true,&quot;generate_images&quot;:false,&quot;publish_mode&quot;:&quot;article_units&quot;&#125;' bind:value={packageOptions} required></textarea></label></div><p class="field-help">The adapter emits progress, a saved-day checkpoint, and a final <code>content-package.v1</code> frame. Choose <code>article_units</code> to publish every ordered unit as its own long-form article with its unit media; <code>feed_item</code> remains available for one compact summary post. Pause asks it to stop at the latest completed checkpoint; resume starts a new run with that checkpoint. Keep executable code and model files on the worker host, outside Swartzit’s core install.</p></div>
                  {:else if runnerKind === 'cross_post'}
                    {#if crossMode === 'recommended'}
                      <div class="runner-subsection"><div class="runner-subsection-heading"><strong>X Recommended timeline</strong><span>Read-only Ego Lite session</span></div><p class="field-help">This recipe visits the signed-in X <strong>For You</strong> timeline and returns one unseen public status per run. It never likes, reposts, follows, messages, or opens compose. Allow-list <code>EGO_BROWSER_SPACE_ID</code> and <code>EGO_BROWSER_CLI</code> on the worker, and keep that dedicated session signed in.</p></div>
                    {:else if crossMode === 'playwright'}
                      <div class="runner-subsection"><div class="runner-subsection-heading"><strong>X Recommended timeline</strong><span>Read-only Ubuntu Playwright session</span></div><p class="field-help">This recipe visits the signed-in X <strong>For You</strong> timeline through Playwright Chromium and returns one unseen public status per run. It never likes, reposts, follows, messages, or opens compose. Install the Playwright browser on the Ubuntu worker, log into the dedicated profile once, and keep <code>X_PLAYWRIGHT_USER_DATA_DIR</code> separate from any personal browser profile.</p></div>
                    {:else}
        ...
        <section class="runner-templates"><div class="runner-templates-heading"><div><p class="runner-eyebrow">STARTER RUNNERS</p><h3>Begin with a known-good recipe</h3><p>These disabled templates use the runner JSON contract and are covered by local tests. Choose one, add an author and community, then save it as a draft. X templates need a worker-side bearer token; Draw Things templates need a local model.</p></div><span class="runner-template-note">Disabled until you enable them</span></div><div class="runner-template-grid">{#each runnerTemplates as template}<article class="runner-template-card"><div class="runner-template-top"><span class="runner-template-mark">{template.kind === 'draw_things' ? '✦' : template.kind === 'content_package' ? '▣' : template.kind === 'cross_post' ? '↗' : template.key === 'batch' ? '2×' : template.key === 'prompt' ? '✎' : '✓'}</span><span class="runner-pill neutral"><span></span>Draft template</span></div><h4>{template.name}</h4><p>{template.description}</p><code>{templateCommandLabel(template)}</code><button class="runner-secondary" onclick={() => useRunnerTemplate(template)} disabled={busy || settings?.modules?.content_runners?.enabled !== true}>Use template</button></article>{/each}</div></section>
          ...
          {:else}
            <div class="runner-list">{#each visibleRunners as runner}
              <article class="runner-card">
                ...
                {/if}
                {#if latestRunError(runner) && (isFailedRunnerRun(latestRun(runner).status) || runner.last_error)}
                  <div class="runner-failure" role="alert"><div><strong>{latestRun(runner).status === 'timeout' ? 'Latest run timed out' : 'Latest run failed'}</strong><p>{latestRunError(runner)}</p>{#if runnerFailureHint(latestRunError(runner))}<small>{runnerFailureHint(latestRunError(runner))}</small>{/if}</div>{#if retryableRunner(runner)}<button type="button" class="runner-secondary" disabled={busy} onclick={() => retryRunner(runner)}>Retry latest run</button>{/if}</div>
                {/if}
                {#if testPreviews(runner).length}
                  <div class="runner-test-result" role="status"><div class="runner-test-result-heading"><div><strong>Test passed · no posts published</strong><small>{testPreviews(runner).length} generated preview{testPreviews(runner).length === 1 ? '' : 's'} are ready to confirm.</small></div><button type="button" class="runner-primary" disabled={busy || runner.state === 'archived'} onclick={() => action('content-runners/' + runner.id + '/run-now', 'The no-publish test passed. Generate and publish this runner now?')}>Confirm &amp; run now</button></div><div class="runner-test-previews">{#each testPreviews(runner) as preview}<article><strong>{preview.title}</strong><p>{preview.generation_config?.prompt || 'Rendered prompt was not recorded.'}</p><small>{testPreviewSettings(preview)}</small></article>{/each}</div></div>
                {/if}
                <div class="runner-card-footer"><div class="runner-card-footer-note">{runner.last_error ? 'Last run needs attention' : runnerIsRunning(runner) ? 'Worker is executing this runner now · ETA is approximate' : runner.test_requested ? 'No-publish test is waiting for the worker' : runner.state === 'draft' ? 'Draft · disabled until you enable it' : runner.state === 'archived' ? 'History retained · no longer scheduled' : runner.next_run_at ? 'Next run ' + date(runner.next_run_at) : 'Ready for a manual run'}</div><div class="runner-card-actions"><button type="button" class="runner-secondary" disabled={busy} onclick={() => editRunner(runner)}>Edit</button><button type="button" class="runner-secondary" aria-label={'Duplicate and edit ' + runner.name} title="Open the same editor with a disabled copy" disabled={busy} onclick={() => duplicateRunner(runner)}>Duplicate &amp; edit</button><button type="button" class="runner-secondary" disabled={busy || runner.state === 'archived'} onclick={() => testRunner(runner.id)}>Test</button><button type="button" class="runner-primary runner-action-primary" disabled={busy || runner.state === 'archived'} onclick={() => action('content-runners/' + runner.id + '/run-now', 'Queue this runner now?')}>Run now</button>{#if runner.kind === 'content_package' && runnerIsRunning(runner)}<button type="button" class="runner-secondary" disabled={busy} onclick={() => controlRunner(latestRun(runner), 'pause')}>Pause at checkpoint</button><button type="button" class="runner-text-action" disabled={busy} onclick={() => controlRunner(latestRun(runner), 'cancel')}>Cancel run</button>{:else if runner.kind === 'content_package' && runner.state === 'paused' && ['paused', 'cancelled'].includes(latestRun(runner).status)}<button type="button" class="runner-primary" disabled={busy} onclick={() => controlRunner(latestRun(runner), 'resume')}>Resume from checkpoint</button>{/if}<button type="button" class="runner-secondary" disabled={busy || runner.state === 'archived'} onclick={() => action('content-runners/' + runner.id + '/toggle', (runner.enabled ? 'Pause' : 'Enable') + ' this runner?')}>{runner.enabled ? 'Pause' : 'Enable'}</button><button type="button" class="runner-text-action" disabled={busy || runner.state === 'archived'} onclick={() => action('content-runners/' + runner.id + '/archive', 'Archive this runner? It will remain in history but cannot run again.')}>Archive</button></div></div>
    ...
    {:else if tab === 'Settings'}
      <div class="admin-columns">
        ...
        <section class="panel">
          <h3>Media storage</h3>
          <p class="muted">Swartzit keeps stable asset IDs in posts while storing originals in a provider you control. The primary is written during the request; each selected secondary is queued independently for background replication and read fallback. The local cache is disposable. Catbox is sharing only and never a backup.</p>
          <div class="admin-toolbar">
            ...
            <fieldset class="media-secondary-choices">
              <legend>Redundant copies</legend>
              {#each [['filesystem', 'Local filesystem'], ['s3', 'S3-compatible storage'], ['ipfs', 'IPFS']] as [provider, label]}
                <label><input type="checkbox" checked={mediaSecondaryValues().includes(provider)} onchange={(event) => toggleMediaSecondary(provider, event.currentTarget.checked)} disabled={busy || settings?.media?.primary === provider} /> {label}</label>
              {/each}
              <small>Choose zero or more providers. The current primary is disabled here.</small>
            </fieldset>
          ...
          <p class="muted">Changing providers affects new writes. Use Migrate media to copy existing assets into the selected primary and all selected secondaries. Set environment credentials or endpoints before selecting a provider; environment overrides are shown as “environment”.</p>
          <div class="admin-toolbar"><button disabled={busy} onclick={() => mediaAction('media/test', 'Test every configured media provider now?')}>Test storage</button><button disabled={busy} onclick={() => mediaAction('media/migrate', 'Copy all media into the configured primary and secondary providers?')}>Migrate media</button><button disabled={busy} onclick={() => mediaAction('media/verify', 'Read and checksum every recorded primary and secondary media replica?')}>Verify media</button><button disabled={busy} onclick={() => mediaAction('media/cache/clear', 'Clear the disposable media cache? Originals will remain safe.')}>Clear cache</button></div>
    ...
    {:else if tab === 'Server'}
      ...
      <section class="panel backup-panel">
        <div class="panel-head backup-heading"><div><h3>Recent backups</h3><span>Archive sizes and checksum readiness</span></div>{#if storage?.backups}<span class={'badge ' + backupStateTone(storage.backups.status)}>{backupStateLabel(storage.backups.status)}</span>{/if}</div>
        {#if storage?.backups?.latest}
          <div class="backup-summary">
            <div><span>Latest archive</span><strong>{size(storage.backups.latest.archive_bytes)}</strong><small>{date(storage.backups.latest.created_at)} · {storage.backups.latest.name}</small></div>
            <div><span>Archive footprint</span><strong>{size(storage.backups.total_archive_bytes)} / {size(storage.backups.budget_bytes)}</strong><small>{number(storage.backups.archive_count)} archives · retention {number(storage.backups.retention)}</small></div>
            <div><span>Latest contents</span><strong>{size(storage.backups.latest.dump_bytes)} + {size(storage.backups.latest.media_bytes)}</strong><small>{storage.backups.latest.checksum_manifest ? 'SHA-256 manifest present' : 'Checksum manifest missing'}</small></div>
          </div>
          <div class="backup-budget"><div class="backup-budget-label"><span>Archive budget</span><strong>{backupPercent(storage.backups).toFixed(1)}%</strong></div><div class="backup-meter"><span style:width={backupPercent(storage.backups) + '%'}></span></div><p class="muted">Warning begins at {size(storage.backups.warning_threshold_bytes)}. This measures stored backup archives, not the live database or media footprint.</p></div>
          <div class="table-wrap backup-table"><table><thead><tr><th>Backup</th><th>Archive</th><th>Database dump</th><th>Media</th><th>Integrity</th></tr></thead><tbody>{#each storage.backups.recent as backup}<tr><td><strong>{date(backup.created_at)}</strong><small>{backup.name}</small></td><td>{size(backup.archive_bytes)}</td><td>{size(backup.dump_bytes)}</td><td>{size(backup.media_bytes)}</td><td><span class={'badge ' + (backup.checksum_manifest ? 'positive' : 'warn')}>{backup.checksum_manifest ? 'SHA-256 manifest' : 'Manifest missing'}</span></td></tr>{/each}</tbody></table></div>
        {:else}
          <p class="muted backup-empty">No backup archives found at <code>{storage?.backups?.directory ?? 'the configured backup directory'}</code>.</p>
        {/if}
      </section>
...
<style>
  ...
  .runner-editor-shell{padding:26px;border-radius:18px;box-shadow:0 12px 30px color-mix(in srgb,var(--heading) 7%,transparent)}.runner-editor-heading{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;padding-bottom:20px;border-bottom:1px solid var(--border)}.runner-editor-heading h3,.runner-list-heading h3,.runner-how-it-works h3{font:600 1.55rem/1.12 Georgia,serif;color:var(--heading);margin:7px 0}.runner-editor-heading p{margin:0}.runner-editor-mode{display:inline-flex;align-items:center;margin-left:7px;padding:4px 7px;border:1px solid var(--border);border-radius:999px;color:var(--accent);font:800 .6rem/1 ui-monospace,monospace;letter-spacing:.08em;vertical-align:middle}.runner-copy-note{display:flex;gap:10px;align-items:baseline;margin-top:18px;padding:12px 14px;border:1px solid color-mix(in srgb,var(--accent) 28%,var(--border));border-radius:10px;background:color-mix(in srgb,var(--accent) 6%,var(--surface));color:var(--muted);font-size:.75rem}.runner-copy-note strong{color:var(--heading);font-size:.78rem}.runner-close{width:34px;height:34px;padding:0!important;border:1px solid var(--border)!important;border-radius:50%!important;background:transparent!important;color:var(--muted)!important;font-size:1.45rem!important;line-height:1;cursor:pointer}.runner-close:hover{color:var(--heading);border-color:var(--accent)!important}.runner-form-alert{display:grid;gap:3px;margin-top:18px;padding:13px 15px;border:1px solid color-mix(in srgb,var(--error) 38%,var(--border));border-radius:11px;background:color-mix(in srgb,var(--error-bg) 70%,var(--surface));color:var(--error);font-size:.8rem}.runner-form-alert strong{font-size:.82rem}.runner-form-alert small{color:var(--muted);font-size:.71rem}.runner-form{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:18px;margin-top:22px}.runner-form-main,.runner-form-side{display:grid;gap:18px;align-content:start}.runner-form-section{min-width:0;border:1px solid var(--border);border-radius:14px;padding:18px;background:color-mix(in srgb,var(--surface) 75%,var(--subtle) 25%)}.runner-form-section legend{padding:0 7px;color:var(--heading);font-size:.88rem;font-weight:800}.runner-form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.runner-form-grid label,.runner-form-section>label,.runner-advanced-body label{display:flex;flex-direction:column;gap:6px;color:var(--heading);font-size:.78rem;font-weight:750}.runner-form-grid small,.runner-form-section small,.field-help{color:var(--muted);font-size:.72rem;font-weight:400}.runner-form-section input,.runner-form-section select,.runner-form-section textarea,.runner-advanced input{width:100%;font:inherit;border:1px solid var(--border);border-radius:8px;background:var(--surface);color:var(--text);padding:10px 11px}.runner-form-section textarea{resize:vertical;min-height:94px}.runner-form-section input:focus,.runner-form-section select:focus,.runner-form-section textarea:focus{border-color:var(--accent)}.runner-prompt{min-height:120px}.runner-command{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:.79rem;min-height:80px}.runner-subsection{display:grid;gap:12px;margin-top:16px;padding-top:16px;border-top:1px solid var(--border)}.runner-subsection-heading{display:flex;justify-content:space-between;gap:10px;align-items:baseline}.runner-subsection-heading strong{color:var(--heading);font-size:.86rem}.runner-subsection-heading span{color:var(--muted);font-size:.72rem}.runner-draw-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.runner-draw-grid .span-two{grid-column:span 2}.runner-lora-editor{display:grid;gap:10px;padding-top:2px}.runner-lora-list{display:grid;gap:9px}.runner-lora-row{display:grid;grid-template-columns:minmax(0,1.8fr) minmax(110px,.7fr) minmax(95px,.55fr) auto;gap:9px;align-items:end;padding:10px;border:1px solid var(--border);border-radius:10px;background:var(--surface)}.runner-lora-row label{display:flex;flex-direction:column;gap:5px;color:var(--heading);font-size:.72rem;font-weight:750}.runner-lora-row input,.runner-lora-row select{width:100%;font:inherit;border:1px solid var(--border);border-radius:7px;background:var(--surface);color:var(--text);padding:8px 9px}.runner-lora-row input:focus,.runner-lora-row select:focus{border-color:var(--accent)}.runner-add-button{padding:7px 10px!important;font-size:.7rem!important}.runner-remove-button{padding:8px 0!important;color:var(--error)!important}.runner-side-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.runner-side-grid label{display:flex;flex-direction:column;gap:5px;color:var(--heading);font-size:.75rem;font-weight:750}.runner-side-grid input{width:100%}.day-picker{display:flex;flex-wrap:wrap;gap:6px;margin-top:16px}.day-picker label{display:flex;align-items:center;gap:5px;padding:7px 8px;border:1px solid var(--border);border-radius:7px;color:var(--muted);font-size:.72rem;font-weight:650;cursor:pointer}.day-picker input{width:auto!important;margin:0}.runner-advanced{border:1px solid var(--border);border-radius:14px;background:var(--surface)}.runner-advanced summary{cursor:pointer;padding:14px 16px;color:var(--heading);font-size:.82rem;font-weight:800}.runner-advanced-body{display:grid;gap:12px;padding:0 16px 16px}.runner-advanced-body label{font-size:.75rem}.runner-advanced-body label:has(input[type=checkbox]){display:flex;flex-direction:row;align-items:center;font-weight:650}.runner-advanced-body input[type=checkbox]{width:auto}.runner-form-actions{grid-column:1/-1;display:flex;align-items:center;gap:10px;padding-top:2px}.runner-form-actions .field-help{margin-left:3px}.runner-form-actions .runner-secondary{background:transparent}
  ...
  @media(max-width:700px){.runner-copy-note{display:grid;gap:3px}.runner-lora-row{grid-template-columns:1fr 1fr}.runner-lora-row label:first-child{grid-column:1/-1}.runner-remove-button{justify-self:start}}
  .runner-permutation-list{display:grid;gap:9px}.runner-permutation-row{display:grid;grid-template-columns:minmax(120px,.7fr) minmax(0,2fr) auto;gap:9px;align-items:end;padding:10px;border:1px solid var(--border);border-radius:10px;background:var(--surface)}.runner-permutation-row label{display:flex;flex-direction:column;gap:5px;color:var(--heading);font-size:.72rem;font-weight:750}.runner-permutation-row input,.runner-permutation-row textarea{width:100%;font:inherit;border:1px solid var(--border);border-radius:7px;background:var(--surface);color:var(--text);padding:8px 9px}.runner-permutation-row textarea{resize:vertical;min-height:58px}.runner-permutation-row input:focus,.runner-permutation-row textarea:focus{border-color:var(--accent)}.runner-permutation-row small{color:var(--muted);font-size:.68rem;font-weight:400}.runner-prompt-permutation-editor{margin-top:14px}.runner-prompt-permutation-editor .runner-subsection-heading>div{display:flex;gap:8px;align-items:baseline}
  @media(max-width:700px){.runner-permutation-row{grid-template-columns:1fr 1fr}.runner-permutation-row label:nth-child(2){grid-column:1/-1}.runner-permutation-row .runner-remove-button{justify-self:start}}
  .runner-test-result{display:grid;gap:11px;margin:0 16px 10px;padding:12px 14px;border:1px solid color-mix(in srgb,#3d9a68 38%,var(--border));border-radius:11px;background:color-mix(in srgb,#3d9a68 7%,var(--surface))}.runner-test-result-heading{display:flex;justify-content:space-between;gap:12px;align-items:center}.runner-test-result-heading strong{display:block;color:var(--heading);font-size:.78rem}.runner-test-result-heading small{display:block;margin-top:3px;color:var(--muted);font-size:.69rem}.runner-test-result-heading button{flex:none;font-size:.7rem!important;padding:8px 11px!important}.runner-test-previews{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px}.runner-test-previews article{min-width:0;padding:9px;border:1px solid var(--border);border-radius:8px;background:var(--surface)}.runner-test-previews strong{display:block;color:var(--heading);font-size:.72rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.runner-test-previews p{margin:5px 0;color:var(--text);font-size:.69rem;line-height:1.35;display:-webkit-box;-webkit-line-clamp:3;line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}.runner-test-previews small{display:block;color:var(--muted);font-size:.65rem;line-height:1.3}
  @media(max-width:700px){.runner-test-result-heading{align-items:flex-start;flex-direction:column}.runner-test-result-heading button{align-self:flex-start}}
  ...
  .backup-panel{margin:18px 0}.backup-heading{align-items:flex-start}.backup-heading h3{margin:0 0 4px;color:var(--heading);font:600 1.2rem Georgia,serif}.backup-heading span{color:var(--muted);font-size:.75rem}.backup-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin:20px 0}.backup-summary>div{min-width:0;padding:14px;border:1px solid var(--border);border-radius:10px;background:color-mix(in srgb,var(--surface) 72%,var(--subtle) 28%)}.backup-summary span,.backup-summary small{display:block;color:var(--muted);font-size:.7rem}.backup-summary strong{display:block;margin:8px 0 5px;color:var(--heading);font:600 1.3rem Georgia,serif;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.backup-summary small{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.backup-budget{margin:4px 0 20px}.backup-budget-label{display:flex;justify-content:space-between;gap:12px;color:var(--muted);font-size:.74rem}.backup-budget-label strong{color:var(--heading)}.backup-meter{height:8px;margin-top:7px;overflow:hidden;border-radius:999px;background:var(--subtle)}.backup-meter span{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--accent),#4d9b68);transition:width .25s ease}.backup-budget p{margin:8px 0 0;font-size:.72rem}.backup-table{overflow-x:auto}.backup-table table{width:100%;border-collapse:collapse;text-align:left;font-size:.78rem}.backup-table th{padding:10px;background:var(--subtle);color:var(--muted);font-size:.68rem;text-transform:uppercase;letter-spacing:.06em;white-space:nowrap}.backup-table td{padding:11px 10px;border-top:1px solid var(--border);vertical-align:top}.backup-table td small{display:block;margin-top:4px;color:var(--muted);font-size:.66rem;white-space:nowrap;max-width:240px;overflow:hidden;text-overflow:ellipsis}.backup-empty{margin:20px 0}.backup-table .badge{font-size:.64rem}
  .media-secondary-choices{display:grid;gap:5px;min-width:210px;margin:0;padding:9px 11px;border:1px solid var(--border);border-radius:8px}.media-secondary-choices legend{padding:0 4px;color:var(--muted);font-size:.76rem;font-weight:750}.media-secondary-choices label{display:flex;align-items:center;gap:6px;color:var(--heading);font-size:.74rem;font-weight:650}.media-secondary-choices input{width:auto}.media-secondary-choices small{color:var(--muted);font-size:.68rem;font-weight:400}
  @media(max-width:800px){.storage-grid{grid-template-columns:1fr}.backup-summary{grid-template-columns:1fr}}
  .runner-failure{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;margin:0 16px 10px;padding:10px 12px;border:1px solid color-mix(in srgb,var(--error) 38%,var(--border));border-radius:10px;background:color-mix(in srgb,var(--error-bg) 58%,var(--surface));color:var(--error)}.runner-failure strong{display:block;font-size:.73rem}.runner-failure p{margin:4px 0 0;color:var(--heading);font:500 .7rem/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}.runner-failure small{display:block;margin-top:5px;color:var(--muted);font-size:.69rem;line-height:1.35}.runner-failure button{flex:none}
  ...
  @media(max-width:700px){.runner-live-progress{margin-left:16px;margin-right:16px}.runner-failure{flex-direction:column}}
