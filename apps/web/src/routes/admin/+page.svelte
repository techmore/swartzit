      ...
      <div class="runner-page">
        ...
        {#if runnerEditorOpen}
          <section id="runner-editor" bind:this={runnerEditorElement} class="runner-editor-shell panel">
            <div class="runner-editor-heading"><div><p class="runner-eyebrow">{runnerEditorMode === 'duplicate' ? 'DUPLICATE CONFIGURATION' : editingRunner ? 'EDIT CONFIGURATION' : 'NEW CONFIGURATION'}</p><h3>{runnerEditorMode === 'duplicate' ? 'Review this copy' : editingRunner ? 'Tune this runner' : 'Create a runner'}</h3><p class="muted">{runnerEditorMode === 'duplicate' ? 'This copy will be saved as a disabled draft. Review the destination and recipe before saving.' : 'Set the destination and recipe first. Reliability controls live in the right column so the important choices stay visible.'}</p></div><button type="button" class="runner-close" aria-label="Close runner editor" onclick={closeRunnerEditor}>×</button></div>
