<script>
  ...
  let runnerName = '', runnerKind = 'command', runnerCommand = '', runnerPrompt = '', runnerAuthor = '', runnerCommunity = '', runnerInterval = 1800, runnerPriority = 100, runnerTimeout = 900, runnerAttempts = 3, runnerBackoff = 60, runnerThreshold = 3, runnerRetention = 30, runnerEnvironmentKeys = '', runnerCaptureOutput = true, runnerMaxLogBytes = 20000, editingRunner = null;
  ...
  async function refresh() {
    ...
    try {
      ...
      const [stats, items, daily, scheduled, history, runnerData, runnerLogData, securityData, uptimeData, settingsData, moderationHistoryData] = await Promise.all([
        ...
        tab === 'Settings' || tab === 'Content Runners' ? api('settings') : Promise.resolve(null),
...
<main class="admin">
  ...
  {:else}
    ...
    {:else if tab === 'Content Runners'}
      <section class="panel"><h3>{editingRunner ? 'Edit runner' : 'Scheduled publishing runners'}</h3><p class="muted">Commands run on the worker host, one at a time, ordered by priority. They must print a JSON post receipt; every result enters moderation. Use a JSON argv array for paths or arguments containing spaces.</p><form class="admin-toolbar" onsubmit={saveRunner}><input placeholder="Runner name" bind:value={runnerName} required maxlength="80" /><select bind:value={runnerKind}><option value="command">Command / generator</option><option value="cross_post">Cross-post adapter</option></select><input placeholder='Command argv JSON, e.g. ["draw-things-cli","generate"]' bind:value={runnerCommand} required /><input placeholder="Prompt" bind:value={runnerPrompt} /><input placeholder="Author handle" bind:value={runnerAuthor} required /><input placeholder="Community slug" bind:value={runnerCommunity} required /><label>Every <input type="number" min="300" max="604800" bind:value={runnerInterval} /> sec</label><label>Priority <input type="number" min="0" max="10000" bind:value={runnerPriority} /></label><label>Timeout <input type="number" min="30" max="86400" bind:value={runnerTimeout} /> sec</label><label>Attempts <input type="number" min="1" max="10" bind:value={runnerAttempts} /></label><label>Backoff <input type="number" min="10" max="86400" bind:value={runnerBackoff} /> sec</label><label>Failure pause <input type="number" min="1" max="100" bind:value={runnerThreshold} /> runs</label><label>Keep logs <input type="number" min="1" max="3650" bind:value={runnerRetention} /> days</label><input placeholder="Worker env keys, comma separated" bind:value={runnerEnvironmentKeys} /><label><input type="checkbox" bind:checked={runnerCaptureOutput} /> Capture stdout/stderr</label><label>Max log bytes <input type="number" min="1024" max="20000" bind:value={runnerMaxLogBytes} /></label><button disabled={busy || settings?.modules?.content_runners?.enabled === false}>{editingRunner ? 'Save changes' : 'Add runner'}</button>{#if editingRunner}<button type="button" onclick={resetRunnerForm}>Cancel edit</button>{/if}</form></section>
      <section class="panel table-wrap"><table><thead><tr><th>Runner</th><th>Destination</th><th>Schedule / policy</th><th>Status</th><th>Actions</th></tr></thead><tbody>{#each runners as runner}<tr><td><strong>{runner.name}</strong><small>{runner.kind} · priority {runner.priority} · config v{runner.config_version}</small></td><td>u/{runner.author}<small>c/{runner.community}</small></td><td>Every {Math.round(runner.interval_seconds / 60)} min<small>{runner.max_attempts} attempts · {runner.timeout_seconds}s timeout · keep {runner.retention_days}d</small></td><td><span class="badge">{runner.state}</span><small>{runner.last_status || 'never run'} · {runner.run_count} runs</small>{#if runner.last_error}<small>{runner.last_error}</small>{/if}{#if runner.paused_reason}<small>{runner.paused_reason}</small>{/if}</td><td><button disabled={busy} onclick={() => editRunner(runner)}>Edit</button><button disabled={busy} onclick={() => action('content-runners/' + runner.id + '/run-now', 'Queue this runner now?')}>Run now</button><button disabled={busy || runner.state === 'archived'} onclick={() => action('content-runners/' + runner.id + '/toggle', (runner.enabled ? 'Pause' : 'Enable') + ' this runner?')}>{runner.enabled ? 'Pause' : 'Enable'}</button><button disabled={busy || runner.state === 'archived'} onclick={() => action('content-runners/' + runner.id + '/archive', 'Archive this runner? It will remain in history but cannot run again.')}>Archive</button></td></tr>{#if logsFor(runner.id).length}<tr><td colspan="5"><details><summary>Recent structured logs</summary><div class="table-wrap"><table><thead><tr><th>Run</th><th>Status</th><th>Attempt</th><th>Duration</th><th>Output / error</th><th>Action</th></tr></thead><tbody>{#each logsFor(runner.id) as log}<tr><td>#{log.id}<small>{date(log.started_at)}</small></td><td><span class="badge">{log.status}</span>{#if log.timed_out}<small>timed out</small>{/if}</td><td>{log.attempt} / {runner.max_attempts}</td><td>{log.duration_ms != null ? log.duration_ms + ' ms' : '—'}</td><td><details><summary>{log.error || (log.stdout ? 'stdout' : 'No output')}</summary><pre>{log.stderr || log.stdout || JSON.stringify(log.detail, null, 2)}</pre></details></td><td><button disabled={busy} onclick={() => action('content-runner-runs/' + log.id + '/replay', 'Replay this runner execution?')}>Replay</button></td></tr>{/each}</tbody></table></div></details></td></tr>{/if}{/each}</tbody></table></section>
