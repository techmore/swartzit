<script>
  ...
  function drawCommand() { return { executable: drawExecutable.trim(), models_dir: drawModelsDir.trim() || undefined, model: drawModel.trim(), width: Number(drawWidth), height: Number(drawHeight), steps: Number(drawSteps), ...(drawCfg === '' || drawCfg == null ? {} : { cfg: Number(drawCfg) }), seed: drawSeed === '' ? null : Number(drawSeed), loras: normalizedLoras(drawLoraRows).filter(row => row.file.trim()).map(row => ({ file: row.file.trim(), ...(row.version.trim() ? { version: row.version.trim() } : {}), ...(row.weight === '' ? {} : { weight: Number(row.weight) }) })), prompt_permutations: drawPromptPermutationRows.map(row => ({ key: row.key.trim(), values: row.values.map(value => value.trim()) })), output_path: drawOutputPath.trim() || undefined, posts_per_run: Number(drawPostsPerRun), title_prefix: drawTitlePrefix.trim() || undefined }; }
