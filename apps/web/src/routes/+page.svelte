<script>
  ...
  async function createPost() { formError = ''; formMessage = ''; const response = await fetch('/api/posts', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` }, body: JSON.stringify({ community, title, body }) }); const result = await response.json(); if (!response.ok) { formError = result.error ?? 'Could not publish discussion'; return; } title = ''; body = ''; formMessage = 'Discussion published.'; }
</script>

<svelte:head><title>Swartzit — the commons</title></svelte:head>
<header><a class="brand" href="/">swartzit</a><span>Read freely. Participate under a pseudonym. Take your community with you.</span><a href="/api/export" download="swartzit-export.json">Export public data</a><a class="login" href="/login">Sign in</a></header>
<main>
