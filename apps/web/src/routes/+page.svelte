...
</div>{#if !redundantSourceTitle(post)}<h3><a href="/post/{post.id}">{post.title}</a></h3>{/if}<p>{post.body}</p>{#if post.source}<SourcePost source={post.source} />{/if}<div class="post-actions"><BookmarkButton id={post.id} /><ShareButton id={post.id} title={post.title} /></div><footer><span>↕ {post.score}</span><span>{post.view_count} views</span><a href="/post/{post.id}">{post.comment_count} comments</a></footer></article>{/each}{/if}<nav aria-label="Discussion pages">{#if data.page > 1}<a href={pageLink(data.page - 1)}>← Previous</a>{/if} {#if data.hasMore}<a href={pageLink(data.page + 1)}>Next →</a>{/if}</nav></section></div>
