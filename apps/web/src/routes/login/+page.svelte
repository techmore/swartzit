...
<main class="auth"><p class="eyebrow">WELCOME BACK</p><h1>Sign in</h1><p class="lede">Your public reading never depended on an account. Sign in when you’re ready to participate.</p><form on:submit|preventDefault={submit}><label>Handle<input bind:value={handle} autocomplete="username" required /></label><label>Password<input type="password" bind:value={password} autocomplete="current-password" required /></label><button>Sign in</button></form>{#if error}<p class="form-error">{error}</p>{/if}{#if message}<p class="form-message">{message}</p>{/if}<p class="switch">New here? <a href="/signup">Create an account</a>.</p></main>
